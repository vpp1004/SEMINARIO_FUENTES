...
alcohol  
```
Determinamos ahora cuales van a ser las comunidades autónomas con mayores casos de alcolemia y las que menos 
```{r}
...
Observamos cuales son las comunidades que tienen mayores casos de cáncer y cuales son las que menos 
...
```
Podemos observar que cataluña, Madrid y Andalucía van a liderar el ranking con las comunidades con mayor número de casos de cancer, mientras que Ceuta y Melilla se encuentran en la cola del ranking.Cabe destacar que al tratarse de número de casos y no de porcentaje, las comunides con mayor población van a ser siempre las que lideren el ránking, determinando que si lo que realmente se hiciera fuera una comparativa de porcentajes,comunidades se encuentran más a la derecha del ránking tendrían mayor porcentaje que las tres comunidades que lo lideran actualmente
...
Generalmente se tiende a decir que teniendo buenos hábitos de vida y viviendo en zonas no contaminadas disminuye el riesgo de padecer enfermedades, pero realmente, no se sabe si esto  tiene más peso que aquellos factores conocidos como factores malos como por ejemplo el alcohol.Hemos realizado un gráficos para comprobar si se produce mayor disminución del riesgo de padecer cáncer a máyor porcentaje de actividad física y porcentaje verde o si el alcohol tiene más peso sobre esto.
...
grafico_final
```
Como podemos observar mientras sube el porcentaje de actividad producido y los casos de cáncer,los casos de alcohol se mantienen prácticamente en el mismo rango, mientras que las zonas verdes influyen verdaderamente muy poco puesto que altos porcentajes de actividad encontramos porcentajes verdes altos y bajos, esto quiere decir que el porcentaje verde de la zona en la que vivamos no influye directamente con el riesgo de producirse cáncer mientras que la actividad y el alcohol influyen de una manera directa.
Para explicar que tiene más peso en la producción del cáncer vamos a realizar un gráfico que relacione las variables verdaderamente influyentes en el la producción de cáncer 
...
Como podemos comprobar el cosumo de alcohol se relaciona más directamente con el número de casos de cáncer que la actividad con su relación, podemos podríaos determinar que tiene más peso lo llamado malo sobre el riesgo de padecer cáncer que las llamadas variables "buenas", en este caso el porcentaje de actividad física.
